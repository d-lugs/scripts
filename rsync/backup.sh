...
check_failure(){
    status=$?
    if [ $status -ne 0 ]; then
        # error 23 - partial transfer due to errors
        if [ $status -eq 23 ]; then
            ...
            failed_files=$(grep -E "(rsync: |IO error encountered|file has vanished)" "$log_file" | \
                ...
                sed -n 's/.*"\(.*\)".*/\1/p' | \
                head -n 20)
            # print error with file list
            if [ -n "$failed_files" ]; then
                error="Rsync backup partially failed (exit code: 23)\nSome files were not backed up:\n\`\`\`\n$failed_files\n\`\`\`"
            else
                error="Rsync backup partially failed (exit code: 23)\nCheck log file: \`$log_file\`"
            fi
        # all other errors
        else
            error="ERROR: Rsync backup failed (exit code: $status)"
        fi
        send_alert "$error"
    else
        send_alert "$(date '+%Y/%m/%d %H:%M:%S') Backup completed ($(($(date +"%s")-$start_time)) s)"
    fi
}
...
if [ "$background" == "true" ]; then
    ...
    # declare all the things & run, redirect stdout to logfile
    bash -c "$(declare -f run_backup); $(declare -f check_failure); $(declare -f send_alert); run_backup" >> "$log_file" 2>&1 &
